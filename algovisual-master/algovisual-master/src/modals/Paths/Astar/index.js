...
const Astar = () => {
    ...
    const start = async (gridSize, grid,setSteps, setSearchTerm, speed, colors, delay, stopRef,gridColor) => {
        ...
        //find path 
        const astr = async (graph, startNode,delay,gridColor,heuristic) => {
            const visited = Array.from(Array(graph.length), () => Array(graph[0].length).fill(0));
            const queue = new PriorityQueue();
            queue.enqueue(startNode,0);
            var path = {};
            
            const gScore = Array.from(Array(graph.length), () => Array(graph[0].length).fill(Number.POSITIVE_INFINITY));
            gScore[startNode[0]][startNode[1]] = 0;
          
            const fScore = Array.from(Array(graph.length), () => Array(graph[0].length).fill(Number.POSITIVE_INFINITY));
            fScore[startNode[0]][startNode[1]] = calculateHeuristic(startNode, endNode);
            ...
            while (queue.length() > 0 && !stopRef.current) {
                ...
                if (visited[currentNode[0]][currentNode[1]] == 0) {
                    ...
                    for (let neighbor of neighbors) {
                        var neighborGScore = gScore[currentNode[0]][currentNode[1]] + 1;

                        if (neighborGScore < gScore[neighbor[0]][neighbor[1]]) {
                        path[neighbor.toString()] = currentNode;
                        gScore[neighbor[0]][neighbor[1]] = neighborGScore;
                        fScore[neighbor[0]][neighbor[1]] = neighborGScore + calculateHeuristic(neighbor, endNode);
                        if (visited[neighbor[0]][neighbor[1]] == 0 && graph[neighbor[0]][neighbor[1]] != 3) {
                            gridColor.current[neighbor[0]][neighbor[1]] = "orange";
                            queue.enqueue(neighbor,fScore[neighbor[0]][neighbor[1]]);
        ...
        var path = await astr(grid, startNode,delay,gridColor);
        if(path !== undefined){
        var i = 1;
    ...
    return (
        <PathMainContainer title={"      A*     "} symbol={symbol} algo={start} />
    );
};
